...
class GameManager:
    ...
    def update(self, events: list):
        ...
        # if self.input.has_changes:
        #     print(self.input)

        if K_LEFT in self.input.justPressed:
            self.current_lane = (self.current_lane - 1) % self.lanes
        elif K_RIGHT in self.input.justPressed:
            self.current_lane = (self.current_lane + 1) % self.lanes
        elif K_LEFTBRACKET in self.input.justPressed:
            self.current_lane = (self.current_flag - 1) % self.flags
        ...
        note_rect = note.sprite.get_rect()
        note_rect.center = self.game.surface.get_rect().center
        self.game.surface.blit(note.sprite, note_rect)
